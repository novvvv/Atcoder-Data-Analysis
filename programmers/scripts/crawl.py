"""Programmers 기업 코딩테스트 문제 목록 수집 (파이프라인 1단계).

출력: data/programmers_data.csv
"""
...
with sync_playwright() as p:
    ...
    page_num = 1
    prev_titles = None  # 마지막 페이지 감지용 — 이전 페이지 제목 목록
    ...
    while True:
        ...
        # 제목 목록이 이전 페이지와 동일하면 페이지네이션 종료
        current_titles = [
            row.select_one("td.title a").text.strip()
            for row in rows
            if row.select_one("td.title a")
        ]
...
print(df.head(10))
df.to_csv("../data/programmers_data.csv", index=False, encoding="utf-8-sig")
print(f"\n총 {len(df)}개 수집 완료!")
